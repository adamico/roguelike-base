...
def collide(col1, col2, &block)
  ...
  col1.each do |i|
    col2.each do |j|
      block.call(i, j) if i.intersect_rect?(j)
    end
  end
end
...
# Name of who make the game
def dev_id
  $gtk.args.cvars['game_metadata.devid'].value
end
...
def tick_flasher(entity)
  return unless entity.flashing

  entity.flash_ticks_remaining -= 1
  entity.merge!(entity.flash_color)

  return unless entity.flash_ticks_remaining <= 0
...
def center_of(entity)
  unless entity.x && entity.y && entity.h && entity.w
    raise StandardError,
          'entity does not have needed properties to find center; must have x, y, w, and h properties'
  end
